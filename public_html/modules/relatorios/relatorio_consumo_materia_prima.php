...
// OBSERVAÇÃO: As opções dos filtros agora são baseadas apenas nos itens que tiveram consumo no período selecionado.
$params_filtros = [$data_inicial_form, $data_final_form];

$sql_linhas = "SELECT DISTINCT gm.id, gm.nome_grupo FROM grupos_maquinas gm JOIN ordens_producao op ON gm.id = op.grupo_id JOIN consumo_producao cp ON op.id = cp.ordem_producao_id WHERE cp.data_consumo BETWEEN ? AND ? ORDER BY gm.nome_grupo ASC";
$linhas_disponiveis = $conn->execute_query($sql_linhas, $params_filtros)->fetch_all(MYSQLI_ASSOC);

$sql_maquinas = "SELECT DISTINCT m.id, m.nome FROM maquinas m JOIN consumo_producao cp ON m.id = cp.maquina_id WHERE cp.data_consumo BETWEEN ? AND ? ORDER BY m.nome ASC";
$maquinas_disponiveis = $conn->execute_query($sql_maquinas, $params_filtros)->fetch_all(MYSQLI_ASSOC);

$sql_pedidos = "SELECT DISTINCT op.numero_pedido FROM ordens_producao op JOIN consumo_producao cp ON op.id = cp.ordem_producao_id WHERE op.numero_pedido IS NOT NULL AND op.numero_pedido != '' AND cp.data_consumo BETWEEN ? AND ? ORDER BY op.numero_pedido DESC";
$pedidos_disponiveis = $conn->execute_query($sql_pedidos, $params_filtros)->fetch_all(MYSQLI_ASSOC);

$sql_produtos = "SELECT DISTINCT p.id, p.nome, p.codigo FROM produtos p JOIN consumo_producao cp ON p.id = cp.produto_material_id WHERE UPPER(p.familia) IN ('MATERIA-PRIMA', 'COMPONENTES') AND cp.data_consumo BETWEEN ? AND ? ORDER BY p.nome ASC";
...
if (isset($_GET['filtrar'])) {
    try {
        ...
        $sql_base = "SELECT 
                     ...
                     WHERE cp.deleted_at IS NULL AND UPPER(p.familia) IN ('MATERIA-PRIMA', 'COMPONENTES')";
        ...
        foreach($dados_relatorio as $dado) {
            ...
            if (!isset($grafico_agregado[$dia_key])) {
                $grafico_agregado[$dia_key] = 0;
            }
            $grafico_agregado[$dia_key] += $dado['total_consumido'];
        }
...
<div class="container mt-4">
    <h2><i class="fas fa-cogs"></i> Consumo de Matéria-Prima/Componentes</h2>

    <div class="card mb-4">
        <div class="card-header">Filtros</div>
        <div class="card-body">
            <form action="relatorio_consumo_materia_prima.php" method="GET">
                <div class="row">
    ...
    if (ctx) {
        ...
        new Chart(ctx, {
            ...
            data: {
                labels: labels,
                datasets: [{
                    label: 'Consumo Diário (Quantidade)',
                    data: data,
