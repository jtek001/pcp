...
$sql_count = "SELECT COUNT(*) AS total_items FROM maquinas" . $where_clause;
...
$sql_fetch = "SELECT * FROM maquinas" . $where_clause . " ORDER BY nome ASC LIMIT ? OFFSET ?";
$params_fetch = array_merge($params, [$items_per_page, $offset]);

$result_fetch = $conn->execute_query($sql_fetch, $params_fetch);
$maquinas = $result_fetch->fetch_all(MYSQLI_ASSOC);

?>

<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-cogs"></i> Gestão de Máquinas</h2>
        <a href="adicionar.php" class="button add"><i class="fas fa-plus"></i> Adicionar Nova</a>
    </div>
            ...
            <tbody>
                <?php foreach ($maquinas as $maquina): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($maquina['descricao']); ?></td>
                        <td><span class="status-<?php echo htmlspecialchars(strtolower($maquina['status'])); ?>"><?php echo htmlspecialchars(ucfirst($maquina['status'])); ?></span></td>
                        <td>
    ...
    <?php else: ?>
        <p style="text-align: center; margin-top: 20px;">Nenhuma máquina encontrada.</p>
    <?php endif; ?>
</div>
