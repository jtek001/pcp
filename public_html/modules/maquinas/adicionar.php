<?php
// modules/maquinas/adicionar.php
// Esta página contém o formulário para adicionar uma nova máquina.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $temp_tag_ativo = sanitizeInput(isset($_POST['tag_ativo']) ? $_POST['tag_ativo'] : '');
    ...
    if (empty($temp_nome) || empty($temp_status)) {
        ...
        $sql_insert = "INSERT INTO maquinas (nome, descricao, status, numero_serie, tag_ativo, fabricante, modelo_maquina, tipo_maquina, localizacao, data_aquisicao, data_ultima_manutencao, capacidade_hora, unidade_capacidade) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            if ($result_insert === TRUE) {
                ...
                $message = "Erro ao cadastrar máquina: " . $conn->error;
                $message_type = "error";
                error_log("Erro ao inserir máquina: " . $conn->error);
            }
        } catch (mysqli_sql_exception $e) {
            $message = "Erro ao cadastrar máquina (SQL): " . $e->getMessage();
            $message_type = "error";
            error_log("Erro fatal ao inserir máquina: " . $e->getMessage());
        }
    }
}
...
<form action="adicionar.php" method="POST">
    <div class="form-group">
        ...
        <input type="text" id="nome" name="nome" value="<?php echo htmlspecialchars($post_values['nome'] ?? ''); ?>" maxlength="100" required placeholder="Ex: CNC Fresa XYZ-3000">
    </div>

    <div class="form-group">
        <label for="status">Status:</label>
        <select id="status" name="status" required>
            <option value="operacional" <?php echo (isset($post_values['status']) && $post_values['status'] == 'operacional') ? 'selected' : ''; ?>>Operacional</option>
            <option value="manutencao" <?php echo (isset($post_values['status']) && $post_values['status'] == 'manutencao') ? 'selected' : ''; ?>>Manutenção</option>
            <option value="parada" <?php echo (isset($post_values['status']) && $post_values['status'] == 'parada') ? 'selected' : ''; ?>>Parada</option>
        </select>
    </div>

    <div class="form-group">
        <label for="numero_serie">Número de Série:</label>
        <input type="text" id="numero_serie" name="numero_serie" value="<?php echo htmlspecialchars($post_values['numero_serie'] ?? ''); ?>" maxlength="50" placeholder="Ex: SN-123456789">
    </div>

    <div class="form-group">
        <label for="tag_ativo">Tag de Ativo:</label>
        <input type="text" id="tag_ativo" name="tag_ativo" value="<?php echo htmlspecialchars($post_values['tag_ativo'] ?? ''); ?>" maxlength="30" placeholder="Ex: MAQ-001">
    </div>

    <div class="form-group">
        <label for="fabricante">Fabricante:</label>
        <input type="text" id="fabricante" name="fabricante" value="<?php echo htmlspecialchars($post_values['fabricante'] ?? ''); ?>" maxlength="100" placeholder="Ex: Siemens">
    </div>

    <div class="form-group">
        <label for="modelo_maquina">Modelo da Máquina:</label>
        <input type="text" id="modelo_maquina" name="modelo_maquina" value="<?php echo htmlspecialchars($post_values['modelo_maquina'] ?? ''); ?>" maxlength="50" placeholder="Ex: XYZ-PRO">
    </div>

    <div class="form-group">
        <label for="tipo_maquina">Tipo de Máquina:</label>
        <input type="text" id="tipo_maquina" name="tipo_maquina" value="<?php echo htmlspecialchars($post_values['tipo_maquina'] ?? ''); ?>" maxlength="50" placeholder="Ex: Fresa CNC">
    </div>

    <div class="form-group">
        <label for="localizacao">Localização:</label>
        <select id="localizacao_select" name="localizacao" data-initial-value="<?php echo htmlspecialchars($post_values['localizacao'] ?? ''); ?>">
            <option value="">Selecione uma Localização</option>
            <?php
            $current_localizacao = $post_values['localizacao'] ?? '';
    ...
    <div class="form-group">
        ...
        <input type="date" id="data_aquisicao" name="data_aquisicao" value="<?php echo htmlspecialchars($post_values['data_aquisicao'] ?? ''); ?>">
    </div>

    <div class="form-group">
        <label for="data_ultima_manutencao">Última Manutenção:</label>
        <input type="date" id="data_ultima_manutencao" name="data_ultima_manutencao" value="<?php echo htmlspecialchars($post_values['data_ultima_manutencao'] ?? ''); ?>">
    </div>

    <div class="form-group">
        <label for="capacidade_hora">Capacidade por Hora:</label>
        <input type="number" id="capacidade_hora" name="capacidade_hora" step="0.01" value="<?php echo htmlspecialchars($post_values['capacidade_hora'] ?? ''); ?>" placeholder="Ex: 150.50">
    </div>

    <div class="form-group">
        <label for="unidade_capacidade">Unidade Capacidade:</label>
        <input type="text" id="unidade_capacidade" name="unidade_capacidade" value="<?php echo htmlspecialchars($post_values['unidade_capacidade'] ?? ''); ?>" maxlength="20" placeholder="Ex: Peças/h">
    </div>

    <div class="form-group full-width">
        <label for="descricao">Descrição Detalhada:</label>
        <textarea id="descricao" name="descricao" placeholder="Detalhes sobre a máquina, especificações técnicas..."><?php echo htmlspecialchars($post_values['descricao'] ?? ''); ?></textarea>
    </div>

    <button type="submit" class="button submit">Cadastrar Máquina</button>
</form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Lógica para alternar entre select e input de texto (reutilizada de produtos)
